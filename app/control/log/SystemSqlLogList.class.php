...
class SystemSqlLogList extends TStandardList
{
    ...
    public function __construct()
    {
        ...
        $this->form->addQuickField('Database', $database,  200);
        ...
        $logdate = $this->datagrid->addQuickColumn('Data', 'logdate', 'center', NULL, new TAction(array($this, 'onReload')), array('order', 'logdate'));
        $login = $this->datagrid->addQuickColumn(_t('Login'), 'login', 'center', NULL, new TAction(array($this, 'onReload')), array('order', 'login'));
        $database = $this->datagrid->addQuickColumn('Database', 'database_name', 'left', NULL, new TAction(array($this, 'onReload')), array('order', 'database'));
